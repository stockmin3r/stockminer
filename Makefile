...
j:
	make -j 12
wasm:
	@cd src/external/libhydrogen && emcc -DNDEBUG -O0 hydrogen.c -o hydrogen.html -DNDEBUG -s EXPORTED_FUNCTIONS='["_hydro_init","_hydro_sign_create","_hydro_pwhash_deterministic","_hydro_pwhash_keygen"]' -s STANDALONE_WASM --no-entry
...
linux:
	sudo apt-get -y install libtool gcc make cmake npm python3-pip zlib1g-dev python3-pbr libssl-dev libcurl4-openssl-dev libcurl4 libcurl3-gnutls libcurl4-openssl-dev libcap2-bin texinfo python3-venv wkhtmltopd libsodium23 libsodium-dev
