...
/* As an exploit mitigation, OpenBSD requires any stacks to be allocated via mmap(...  MAP_STACK ...).
 * Also enable this on debug builds to catch stack overflows while testing (MAP_STACK exists in Linux, but it's a no-op).  */
...
#ifdef __x86_64__
/* See comment in coro_reset() for an explanation of why this routine is necessary. */
...
#define STACK_PTR 4
    ...
    libucontext_makecontext(&coro->context, (void (*)())coro_entry_point, 3, coro, func, data);
...
#if defined(INSTRUMENT_FOR_VALGRIND)
    coro->vg_stack_id = VALGRIND_STACK_REGISTER(coro->stack, (char *)coro->stack + CORO_STACK_SIZE);
#endif
    return coro;
}
...
ALWAYS_INLINE int64_t coro_resume(struct coro *coro)
{
    ...
    assert(coro->context[STACK_PTR] >= (uintptr_t)coro->stack &&
           coro->context[STACK_PTR] <= (uintptr_t)(coro->stack + CORO_STACK_SIZE));
#endif
...
#endif
#if defined(ALLOCATE_STACK_WITH_MMAP)
    munmap(coro->stack, CORO_STACK_SIZE);
#endif
    free(coro);
}
...
static void coro_defer_disarm_internal(struct coro *coro, struct coro_defer *defer)
{
...
{
    assert(d >= 0);
    return coro_defer_disarm_internal(coro, coro_defer_array_get_elem(&coro->defer, (size_t)d));
}
...
#endif
#if defined(INSTRUMENT_FOR_VALGRIND) || defined(INSTRUMENT_FOR_ASAN)
    coro_defer2(coro, instrument_bpa_free, ptr, (void *)(uintptr_t)requested_size);
#endif
...
void *coro_malloc(struct coro *coro, size_t size)
{
    ...
    const size_t aligned_size = (size + sizeof(void *) - 1ul) & ~(sizeof(void *) - 1ul);
...
#if defined(INSTRUMENT_FOR_ASAN)
        __asan_poison_memory_region(coro->bump_ptr_alloc.ptr, CORO_BUMP_PTR_ALLOC_SIZE);
#endif
#if defined(INSTRUMENT_FOR_VALGRIND)
        VALGRIND_MAKE_MEM_NOACCESS(coro->bump_ptr_alloc.ptr,  CORO_BUMP_PTR_ALLOC_SIZE);
#endif
