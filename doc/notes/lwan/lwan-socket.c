...
static int listen_addrinfo(int fd, const struct addrinfo *addr, bool print_listening_msg, bool is_https)
{
    listen(fd, get_backlog_size());
    if (print_listening_msg) {
        const char *s_if_https = is_https ? "s" : "";
        char host_buf[NI_MAXHOST], serv_buf[NI_MAXSERV];

        getnameinfo(addr->ai_addr, addr->ai_addrlen, host_buf, sizeof(host_buf), serv_buf, sizeof(serv_buf), NI_NUMERICHOST | NI_NUMERICSERV);
        if (addr->ai_family == AF_INET6)
            lwan_status_info("Listening on http%s://[%s]:%s", s_if_https, host_buf, serv_buf);
        else
            lwan_status_info("Listening on http%s://%s:%s",   s_if_https, host_buf, serv_buf);
    }
...
static int bind_and_listen_addrinfos(const struct addrinfo *addrs, bool print_listening_msg, bool is_https)
{
...
static int setup_socket_normally(const struct lwan *l, bool print_listening_msg, bool is_https, const char *listener_from_config)
{
    char            *node, *port;
...
{
    SET_SOCKET_OPTION(SOL_SOCKET, SO_LINGER, (&(struct linger){.l_onoff = 1, .l_linger = 1}));
#ifdef __linux__
    ...
    SET_SOCKET_OPTION_MAY_FAIL(SOL_TCP, TCP_DEFER_ACCEPT, (int[]){(int)l->config.keep_alive_timeout});
...
static int from_systemd_socket(const struct lwan *l, int fd)
{
    if (!sd_is_socket_inet(fd, AF_UNSPEC, SOCK_STREAM, 1, 0))
        lwan_status_critical("Passed file descriptor is not a listening TCP socket");
    return set_socket_options(l, set_socket_flags(fd));
}

int lwan_create_listen_socket(const struct lwan *l, bool print_listening_msg, bool is_https)
{
    const char *listener = is_https ? l->config.tls_listener : l->config.listener;
    ...
    if (!strncmp(listener, "systemd:", sizeof("systemd:") - 1)) {
        ...
        if (n < 0) {
            errno = -n;
            lwan_status_perror("Could not parse socket activation data from systemd");
            return n;
        }
        ...
        strv_free(names);
        if (fd < 0)
            lwan_status_critical("No socket named `%s' has been passed from systemd", listener);
    ...
    if (streq(listener, "systemd")) {
        ...
        if (n != 1)
            lwan_status_critical("%d listeners passed from systemd. Must specify listeners with systemd:listener-name syntax", n);
