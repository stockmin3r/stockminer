 ...
 * extra info:
 *
 * https://d2-naver-com.translate.goog/helloworld/267396?_x_tr_sl=ko&_x_tr_tl=en&_x_tr_hl=en&_x_tr_pto=sc&_x_tr_hist=true - Fantastic Timer Wheel descriptions
 * https://lwn.net/Articles/646950/ - Linux Kernel Timer Wheel
 */
...
struct timeout {
    ...
    struct list_head *pending; /* timeout list if pending on wheel or expiry queue */
    struct list_node  tqe;     /* entry member for struct timeout_list lists */
};

struct timeout  *timeout_init    (struct timeout  *);            /* initialize timeout structure (same as TIMEOUT_INITIALIZER) */
struct timeouts *timeouts_open   (timeout_error_t *);            /* open a new timing wheel, setting optional HZ (for float conversions) */
void             timeouts_close  (struct timeouts *);            /* destroy timing wheel */
void             timeouts_update (struct timeouts *, timeout_t); /* update timing wheel with current absolute time */
timeout_t        timeouts_timeout(struct timeouts *);            /* return interval to next required update */
void             timeouts_add    (struct timeouts *, struct timeout *, timeout_t); /* add timeout to timing wheel */
void             timeouts_del    (struct timeouts *, struct timeout *);            /* remove timeout from any timing wheel or expired queue (okay if on neither) */
struct timeout  *timeouts_get    (struct timeouts *);            /* return any expired timeout (caller should loop until NULL-return) */
...
void timeouts_update(struct timeouts *T, abstime_t curtime)
{
    ...
    for (wheel = 0; wheel < WHEEL_NUM; wheel++) {
        ...
        if ((elapsed >> (wheel * WHEEL_BIT)) > WHEEL_MAX) {
            ...
            pending |= rotr(rotl(((WHEEL_C(1) << _elapsed) - 1), nslot), (unsigned int)_elapsed);
...
static timeout_t timeouts_int(struct timeouts *T)
{
    ...
    for (wheel = 0; wheel < WHEEL_NUM; wheel++) {
        if (T->pending[wheel]) {
            ...
            /* ctz input cannot be zero: T->pending[wheel] is nonzero, so rotr() is nonzero. */
