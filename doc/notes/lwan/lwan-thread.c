...
lwan_setup_tls_keys(int fd, const mbedtls_ssl_context *ssl, int rx_or_tx)
{
    ...
    setsockopt(fd, SOL_TLS, rx_or_tx, &info, sizeof(info));
...
static bool lwan_setup_tls(const struct lwan *l, struct lwan_connection *conn)
{
    ...
    mbedtls_ssl_setup(&ssl, &l->tls->config);
     ...
     * opportunity to free the SSL context.  Defer this call for these  cases. */
    coro_deferred defer = coro_defer(conn->coro, lwan_setup_tls_free_ssl_context, &ssl);
    if (UNLIKELY(!defer)) {
    ...
    mbedtls_ssl_set_bio(&ssl, &ctx, lwan_mbedtls_send, lwan_mbedtls_recv, NULL);
...
__attribute__((noreturn)) static int process_request_coro(struct coro *coro, void *data)
{
    ...
    while (true) {
        ...
        struct lwan_request request = {.global_response_headers = &lwan->headers,
                                       .conn     = conn,
                                       .fd       = fd,
        ...
        /* Ensure string buffer is reset between requests, and that the backing store isn't over 2KB. */
                               ...
                               enum lwan_connection_coro_yield yield_result)
{
    ...
    struct epoll_event event = {.events = conn_flags_to_epoll_events(conn->flags), .data.ptr = conn};
...
static inline void resume_coro(struct timeout_queue *tq, struct lwan_connection *conn, int epoll_fd)
{
...
static void update_date_cache(struct lwan_thread *thread)
{
    ...
    lwan_format_rfc_time(now + (time_t)thread->lwan->config.expires, thread->date.expires);
}
                                ...
                                enum lwan_http_status         status)
{
    ...
    if (!send_string_without_coro(fd, lwan_http_status_as_string_with_code(status), MSG_MORE))
        goto shutdown_and_close;
    ...
    if (send_buffer_without_coro(fd, l->headers.value, l->headers.len, MSG_MORE)) {
        struct lwan_strbuf buffer;

        lwan_strbuf_init(&buffer);
        lwan_fill_default_response(&buffer, status);
        send_buffer_without_coro(fd, lwan_strbuf_get_buffer(&buffer), lwan_strbuf_get_length(&buffer), 0);
...
bool spawn_coro(struct lwan_connection *conn, struct coro_switcher *switcher, struct timeout_queue *tq)
{
    ...
    assert((uintptr_t)t <  (uintptr_t)(tq->lwan->thread.threads + tq->lwan->thread.count));
...
static bool accept_waiting_clients(const struct lwan_thread *t, const struct lwan_connection *listen_socket)
{
    ...
    while (true) {
        ...
        if (LIKELY(fd >= 0)) {
            ...
            if (UNLIKELY(r < 0)) {
                lwan_status_perror("Could not add file descriptor %d to epoll set %d. Dropping connection", fd, conn->thread->epoll_fd);
...
static int create_listen_socket(struct lwan_thread *t, unsigned int num, bool tls)
{
    ...
    if (num == 0) {
        ...
        setsockopt(listen_fd, SOL_SOCKET, SO_ATTACH_REUSEPORT_CBPF, &fprog, sizeof(fprog));
        setsockopt(listen_fd, SOL_SOCKET, SO_LOCK_FILTER, (int[]){1}, sizeof(int));
    }
#elif defined(LWAN_HAVE_SO_INCOMING_CPU) && defined(__x86_64__)
    setsockopt(listen_fd, SOL_SOCKET, SO_INCOMING_CPU, &t->cpu, sizeof(t->cpu));
#endif
...
static bool process_pending_timers(struct timeout_queue *tq, struct lwan_thread *t, int epoll_fd)
{
    ...
    if (should_expire_timers) {
        ...
        /* tq timeout expires every 1000ms if there are connections, so  update the date cache at this point as well.  */
        update_date_cache(t);
...
static int turn_timer_wheel(struct timeout_queue *tq, struct lwan_thread *t, int epoll_fd)
{
    const int infinite_timeout = -1;
    timeout_t wheel_timeout;
    struct    timespec now;

    clock_gettime(monotonic_clock_id, &now);
    timeouts_update(t->wheel, (timeout_t)(now.tv_sec * 1000 + now.tv_nsec / 1000000));

    /* Check if there's an expired timer. */
    wheel_timeout = timeouts_timeout(t->wheel);
    if (wheel_timeout > 0)
        return (int)wheel_timeout; /* No, but will soon. Wake us up in wheel_timeout ms. */
...
static void *thread_io_loop(void *data)
{
    ...
    lwan_status_debug("Worker thread #%zd starting", t - t->lwan->thread.threads + 1);
...
static void create_thread(struct lwan *l, struct lwan_thread *thread)
{
    ...
    pthread_create(&thread->self, &attr, thread_io_loop, thread);
...
adjust_thread_affinity(const struct lwan_thread *thread)
{
    ...
    pthread_setaffinity_np(thread->self, sizeof(set), &set);
}
...
static bool lwan_init_tls(struct lwan *l)
{
    static const int aes128_ciphers[] = {
        /* Only allow Ephemeral Diffie-Hellman key exchange, so Perfect Forward Secrecy is possible.  */
    ...
    if (!is_tls_ulp_supported())
        lwan_status_critical("TLS ULP not loaded. Try running `modprobe tls` as root.");
    ...
    mbedtls_ssl_conf_own_cert  (&l->tls->config, &l->tls->server_cert, &l->tls->server_key);
    mbedtls_ctr_drbg_seed      (&l->tls->ctr_drbg, mbedtls_entropy_func, &l->tls->entropy, NULL, 0);
    mbedtls_ssl_config_defaults(&l->tls->config, MBEDTLS_SSL_IS_SERVER, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT);

    mbedtls_ssl_conf_rng          (&l->tls->config, mbedtls_ctr_drbg_random, &l->tls->ctr_drbg);
    mbedtls_ssl_conf_ciphersuites (&l->tls->config, aes128_ciphers);
    mbedtls_ssl_conf_renegotiation(&l->tls->config, MBEDTLS_SSL_RENEGOTIATION_DISABLED);
    mbedtls_ssl_conf_legacy_renegotiation(&l->tls->config, MBEDTLS_SSL_LEGACY_NO_RENEGOTIATION);
...
void lwan_thread_init(struct lwan *l)
{
    ...
    if (l->online_cpus > 1) {
        static_assert(sizeof(struct lwan_connection) == 32, "Two connections per cache line");
        lwan_status_debug("%d CPUs of %d are online. Reading topology to pre-schedule clients", l->online_cpus, l->available_cpus);
        /*
         * Pre-schedule each file descriptor, to reduce some operations in the fast path.
    ...
    for (unsigned int i = 0; i < l->thread.count; i++) {
        ...
        if (schedtbl) {
            ...
            if (!thread) {
                ...
                lwan_status_critical("Could not figure out which CPU thread %d should go to", i);
            }
        } else {
            thread = &l->thread.threads[i % l->thread.count];
        }

        pthread_barrier_init(&l->thread.barrier, NULL, 2);
        create_thread(l, thread);
        thread->listen_fd = create_listen_socket(thread, i, false);
        l->conns[thread->listen_fd].flags |= CONN_LISTENER;

        if (tls_initialized) {
            thread->tls_listen_fd = create_listen_socket(thread, i, true);
