...
{
    /* Placeholder handler so that __start_lwan_handler and __stop_lwan_handler symbols will get defined. */
...
{
    /* NOTE: Update lwan_prepare_response_header_full() in lwan-response.c if new headers are added here. */
    ...
    if (strcaseequal_neutral(name, "Transfer-Encoding"))
        return false;
    if (!strncasecmp(name, "Access-Control-Allow-", sizeof("Access-Control-Allow-") - 1))
        return false;
    return true;
}

static void build_response_headers(struct lwan *l, const struct lwan_key_value *kv)
{
...
static void parse_global_headers(struct config *c, struct lwan *lwan)
{
    ...
    while ((l = config_read_line(c))) {
        ...
        case CONFIG_LINE_TYPE_SECTION:
            config_error(c, "No sections are supported under the 'headers' section");
            goto cleanup;

        case CONFIG_LINE_TYPE_LINE:
            kv = lwan_key_value_array_append(&hdrs);
            if (!kv)
                lwan_status_critical_perror("Could not allocate memory for custom response header");
...
static void parse_listener_prefix_authorization(struct config *c, struct config_line *l, struct lwan_url_map *url_map)
{
...
add_map:
    ...
    if (handler) {
        ...
        lwan_status_debug("Initializing module %s from config", get_module_name(module));
        ...
        config_error(c, "Module isn't prepared to load settings from a file; create_from_hash() method isn't present");
        goto out;
...
static void register_url_map(struct lwan *l, const struct lwan_url_map *map)
{
    ...
    if (copy->module && copy->module->create) {
        lwan_status_debug("Initializing module %s from struct", get_module_name(copy->module));

        copy->data = copy->module->create(map->prefix, copy->args);
        if (!copy->data)
            lwan_status_critical("Could not initialize module %s", get_module_name(copy->module));
...
static bool setup_from_config(struct lwan *lwan, const char *path)
{
    ...
    while ((line = config_read_line(conf))) {
        ...
        case CONFIG_LINE_TYPE_LINE:
            if (streq(line->key, "keep_alive_timeout")) {
                ...
                if (n_threads < 0)
                    config_error(conf, "Invalid number of threads: %ld", n_threads);
                ...
                if (max_put_data_size < 0)
                    ...
                    config_error(conf, "Maximum put data can't be over 128MiB");
    ...
    if (config_last_error(conf)) {
        lwan_status_critical("Error on config file \"%s\", line %d: %s", path, config_cur_line(conf), config_last_error(conf));
                                  ...
                                  const struct lwan_config *config)
{
    if (!setup_from_config(l, config->config_file_path)) {
        if (config->config_file_path)
            lwan_status_critical("Could not read config file: %s", config->config_file_path);
    }
...
static void get_number_of_cpus(struct lwan *l)
{
    ...
    if (n_available_cpus < 0) {
        lwan_status_warning("Could not get number of available CPUs, assuming %ld CPUs", n_online_cpus);
...
void lwan_init_with_config(struct lwan *l, const struct lwan_config *config)
{
    ...
    /* Initialize status first, as it is used by other things during their initialization. */
    lwan_status_init(l);
    ...
    if (!l->config.n_threads) {
        ...
        l->thread.count = 256;
        lwan_status_warning("%d threads requested, but max 256 supported", l->config.n_threads);
    } else {
    ...
    lwan_status_info("Using %d threads, maximum %d sockets per thread", l->thread.count, l->thread.max_fd);
